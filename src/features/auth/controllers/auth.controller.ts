...
import { AuthResponseDto } from '../application/dto/auth-response.dto';@Controller('auth')
export class AuthController {
  constructor(private readonly authService: AuthService) {}

  @Post('signup')
  @Throttle({ short: { ttl: 60000, limit: 5 } }) // 5 signups per minute
  @HttpCode(HttpStatus.CREATED)  async signup(@Body() signupDto: SignupDto): Promise<AuthResponseDto> {
  ...
  @HttpCode(HttpStatus.OK)  async login(@Body() loginDto: LoginDto): Promise<AuthResponseDto> {
    return this.authService.login(loginDto);
  }

  @Post('refresh')
  @Throttle({ short: { ttl: 60000, limit: 10 } }) // 10 token refreshes per minute
  @HttpCode(HttpStatus.OK)  async refresh(
  ...
  @HttpCode(HttpStatus.OK)  async logout(@Request() req): Promise<{ message: string }> {
