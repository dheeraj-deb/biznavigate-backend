...
export class WhatsAppService {
        ...
        for (const change of changes) {
          ...
          if (messages.length || statuses.length) {
            const statusSummary = statuses
              .map((s: any) => `${s.id}:${s.status}:${s.recipient_id ?? 'unknown'}`)
              .join(', ');
            this.logger.log(
          ...
          if (statuses.length > 0) {
            for (const status of statuses) {
              await this.handleStatusWebhook(status, value.metadata);
            }
          }
        }
      }
  ...
  private async updateCampaignRecipientStatus(
    ...
    if (recipient.status === 'READ' && campaignStatus === 'DELIVERED') {
      this.logger.debug(
        `[CampaignDelivery] Ignoring stale delivered status for already-read recipient ${recipient.id}`,
      );
      return;
    }
