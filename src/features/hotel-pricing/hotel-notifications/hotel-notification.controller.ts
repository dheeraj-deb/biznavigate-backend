...
import { HotelNotificationService } from './hotel-notification.service';@UseGuards(JwtAuthGuard)
...
export class HotelNotificationController {
  ...
  @Get('unread-count')  unreadCount(@Request() req) {
    return this.notificationService.getUnreadCount(req.user.business_id);
  }

  @Patch(':id/read')
  @HttpCode(HttpStatus.OK)  markRead(@Request() req, @Param('id') id: string) {
    return this.notificationService.markRead(req.user.business_id, id);
  }

  @Patch('read-all')
  @HttpCode(HttpStatus.OK)  markAllRead(@Request() req) {
