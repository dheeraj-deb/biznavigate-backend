...
export class WhatsAppTemplatesService {
    ...
    async submit(businessId: string, templateId: string) {
        ...
        this.logger.log(`[Template] gupshupAppId=${gupshupAppId} tokenPrefix=${gupshupToken?.slice(0, 20)}`);
        ...
        try {
            if (gupshupToken && gupshupAppId) {
                ...
                // Must be globally unique across Gupshup — append appId suffix to avoid collisions
                const baseName = template.name.toLowerCase().replace(/[^a-z0-9_]/g, '_').replace(/^_+|_+$/g, '');
                // Use last 8 chars of appId as unique suffix
                ...
                params.set('templateType', template.components.header?.type === 'IMAGE' ? 'IMAGE'
                    ...
                    : 'TEXT');
                params.set('vertical', template.category); // business vertical — use category as fallback
                params.set('content', bodyText);
                params.set('example', exampleBody); // required field
                ...
                this.logger.log(`[Template] Submitting to Gupshup — elementName=${elementName} appId=${gupshupAppId} params=${params.toString()}`);
                ...
                // Gupshup returns { status: 'success', template: { id, status, elementName, ... } }
                const gTemplate = data?.template ?? data;
                metaResult = { id: gTemplate?.id ?? gTemplate?.elementName ?? template.name, status: gTemplate?.status ?? 'PENDING' };
            } else {
    ...
    async syncFromMeta(businessId: string) {
        ...
        for (const mt of metaTemplates) {
            ...
            // Match by metaTemplateId first, then fall back to name+language (covers Gupshup-submitted templates)
            const existing = await this.templateModel.findOne({
                businessId,
                isDeleted: false,
                $or: [
            ...
            if (existing) {
                await this.templateModel.findByIdAndUpdate(existing._id, {
                    status,
                    metaTemplateId: String(mt.id), // update in case it was missing
