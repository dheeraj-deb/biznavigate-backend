...
import { WhatsAppProviderAccount, WhatsAppProviderSendService } from '../outbound/whatsapp-provider-send.service';

type CatalogSendAccount = WhatsAppProviderAccount;
...
export class WhatsAppCatalogOrderService {
  ...
  async handleCatalogOrder(orderPayload: any, metadata: any, contacts: any[]): Promise<void> {
    try {
      ...
      for (const item of productItems) {
        ...
        try {
          await this.prisma.product_inquiries.create({
            data: {
              business_id: account.business_id,
              tenant_id: account.businesses.tenant_id,
      ...
      // Get updated cart
      const cart = await this.cartService.getCart(customer.customer_id, account.business_id);
  ...
  private async sendCartConfirmation(
    ...
    try {
      ...
      await this.providerSendService.sendViaProvider(account, to, messagePayload);
    } catch (error) {
